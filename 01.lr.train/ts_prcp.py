...
def load_data():
    ...
    bigPr_linear_tr = np.zeros((15330, Nlat, Nlon), dtype='float32')
    bigPr_ground_pr = np.zeros((5110, Nlat, Nlon), dtype='float32')
    bigPr_linear_pr = np.zeros((5110, Nlat, Nlon), dtype='float32')
    for j in range(Nlat):
        for i in range(Nlon):
            if m[j, i]:
                dataid = "%05i" % (j * Nlon + i)
                print("%s: In China！！j=%i, i=%i" % (dataid, j, i))
                fname = "./out/lr_data." + dataid
                ...
                bigPr_linear_tr[:, j, i] = data_linear_tr[:, 0]
                bigPr_linear_pr[:, j, i] = data_linear_pr[:, 0]

    return bigPr_linear_tr, bigPr_linear_pr
...
if __name__ == '__main__':
    ...
    # Combine
    bigPr_linear_tr, bigPr_linear_pr = load_data()

    # Dump and Load
    with open('China_linear_prcp.pkl', 'wb') as f:
        pickle.dump({'bigPr_linear_tr': bigPr_linear_tr,                 
                     'bigPr_linear_pr': bigPr_linear_pr}, f)
