...
def weight_mean(big_historical):
...
if __name__ == '__main__':
    ...
    with open('../../01.ann.train/China_his_prcp.pkl', 'rb') as f:
        data_dict = pickle.load(f)
    bigPr_ann_his = data_dict['bigPr_ann_his']

    with open('../../01.lr.train/China_his_prcp.pkl', 'rb') as f:
        data_dict = pickle.load(f)
    bigPr_linear_his = data_dict['bigPr_linear_his']

    with open('../../01.poisson.train/China_his_prcp.pkl', 'rb') as f:
        data_dict = pickle.load(f)
    ...
    bigPr_bc_his = bigPr_bc_his[:, :, 99, 185]
    bigPr_ann_his = bigPr_ann_his[:, :, 99, 185]
    bigPr_linear_his = bigPr_linear_his[:, :, 99, 185]
    bigPr_poisson_his = bigPr_poisson_his[:, :, 99, 185]
    bigPr_ccsm_his = bigPr_ccsm_his.values.reshape(56, 365, 42, 57)
    bigPr_ccsm_his = bigPr_ccsm_his[:, :, 26, 37]

    bigPr_bc_his = monthlymean(bigPr_bc_his)
    bigPr_ann_his = monthlymean(bigPr_ann_his)
    bigPr_linear_his = monthlymean(bigPr_linear_his)
    ...
    bigPr_cru_his = bigPr_cru_his[20:50, :, 259, 592]

    mm = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    for i in range(12):
        bigPr_cru_his[:, i] = bigPr_cru_his[:, i] / mm[i]

    bigPr_bc_his, bigPr_ann_his, bigPr_linear_his, bigPr_poisson_his, bigPr_ccsm_his = \
        bigPr_bc_his.reshape(-1) * 86400, \
        bigPr_ann_his.reshape(-1) * 86400, \
        bigPr_linear_his.reshape(-1) * 86400, \
        bigPr_poisson_his.reshape(-1) * 86400, \
        bigPr_ccsm_his.reshape(-1) * 86400

    bigPr_ann_his = bigPr_ann_his / 1e7
    bigPr_linear_his = bigPr_linear_his / 1e7

    with open('Beijing_talyor_ccsm.pkl', 'wb') as f:
        pickle.dump({'bigPr_bc_his': bigPr_bc_his, 
                     'bigPr_ann_his': bigPr_ann_his,
                     'bigPr_linear_his': bigPr_linear_his,
