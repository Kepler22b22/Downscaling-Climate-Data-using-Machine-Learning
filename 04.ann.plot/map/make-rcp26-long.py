...
if __name__=='__main__':
    ...
    with open('../../04.ann.BiasCorrection/China_tmax_post_rcp26.pkl', 'rb') as f:
        data_dict = pickle.load(f)
    bigMax_ann_rcp26 = data_dict['bigMax_ann_rcp26']

    with open('../../04.ann.BiasCorrection/China_tmin_post_rcp26.pkl', 'rb') as f:
        data_dict = pickle.load(f)
    ...
    bigMin_ann_rcp26 = np.squeeze(np.array(bigMin_ann_rcp26))
    
    bigMax_ccsm_rcp26, bigMin_ccsm_rcp26 = \
        bigMax_ccsm_rcp26.values.reshape(95, 365, 42, 57), bigMin_ccsm_rcp26.values.reshape(95, 365, 42, 57)

    bigMax_ann_rcp26, bigMin_ann_rcp26 = \
        monthlymean(bigMax_ann_rcp26), monthlymean(bigMin_ann_rcp26)

    bigMax_ccsm_rcp26, bigMin_ccsm_rcp26 = \
        monthlymean(bigMax_ccsm_rcp26), monthlymean(bigMin_ccsm_rcp26)
    ...
    #            continue
    #        else:
    #            bigMax_ann_rcp26[:, :, j, i] = np.nan
    #            bigMin_ann_rcp26[:, :, j, i] = np.nan
    ...
    yapr_max = np.mean(bigMax_ann_rcp26[64:94, 6, :, :], axis=0)
    yapr_min = np.mean(bigMin_ann_rcp26[64:94, 0, :, :], axis=0)
    ycpr_max = np.mean(bigMax_ccsm_rcp26[64:94, 6, :, :], axis=0)
    ycpr_min = np.mean(bigMin_ccsm_rcp26[64:94, 0, :, :], axis=0)
    ...
    my.plot2d_map(yCpr_max, Lon, Lat,
                  levs = np.arange(-6.5, 6.5, 0.5),
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmax-ccsm-rcp26-2.pdf' )

    my.plot2d_map(yApr_max, lon, lat,
                  levs = np.arange(-6.5, 6.5, 0.5), 
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmax-ann-rcp26-2.pdf' )
    ...
    my.plot2d_map(yCpr_min, Lon, Lat,
                  ...
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmin-ccsm-rcp26-2.pdf' )

    my.plot2d_map(yApr_min, lon, lat,
                  levs = np.arange(-6.5, 6.5, 0.5), 
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmin-ann-rcp26-2.pdf' )
