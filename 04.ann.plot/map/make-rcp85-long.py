...
if __name__=='__main__':
    ...
    bigMax_ccsm_rcp85, bigMin_ccsm_rcp85 = \
        bigMax_ccsm_rcp85.values.reshape(95, 365, 42, 57), bigMin_ccsm_rcp85.values.reshape(95, 365, 42, 57)
    ...
    yapr_max = np.mean(bigMax_ann_rcp85[64:94, 6, :, :], axis=0)
    yapr_min = np.mean(bigMin_ann_rcp85[64:94, 0, :, :], axis=0)
    ycpr_max = np.mean(bigMax_ccsm_rcp85[64:94, 6, :, :], axis=0)
    ycpr_min = np.mean(bigMin_ccsm_rcp85[64:94, 0, :, :], axis=0)
    ...
    my.plot2d_map(yCpr_max, Lon, Lat,
                  levs = np.arange(-9, 10, 1),
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmax-ccsm-rcp85-2.pdf' )

    my.plot2d_map(yApr_max, lon, lat,
                  levs = np.arange(-9, 10, 1), 
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmax-ann-rcp85-2.pdf' )
    ...
    my.plot2d_map(yCpr_min, Lon, Lat,
                  ...
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmin-ccsm-rcp85-2.pdf' )

    my.plot2d_map(yApr_min, lon, lat,
                  levs = np.arange(-12, 13, 1), 
                  domain = [17, 54, 70, 140],
                  cbar = 'seismic', cbarstr = '',
                  ti='', fout = './figures/subplot-tmin-ann-rcp85-2.pdf' )
