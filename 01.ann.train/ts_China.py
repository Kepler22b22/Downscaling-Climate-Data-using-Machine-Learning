...
def load_data():
    bigPr_ann_tr = np.zeros((15330, Nlat, Nlon), dtype='float32')
    bigPr_ann_pr = np.zeros((5110, Nlat, Nlon), dtype='float32')
    for j in range(Nlat):
        for i in range(Nlon):
            if m[j, i]:
                dataid = "%05i" % (j * Nlon + i)
                print("%s: In China！！j=%i, i=%i" % (dataid, j, i))
                fname = "./out_prcp/ann_data_prcp." + dataid
		...
		# 2. put into Big array
                bigPr_ann_tr[:, j, i] = data_ann_tr[:]
                bigPr_ann_pr[:, j, i] = data_ann_pr[:]

    return bigPr_ann_tr, bigPr_ann_pr
...
if __name__ == '__main__':
    ...
    # Combine
    bigPr_ann_tr, bigPr_ann_pr = load_data()

    # Dump and Load
    with open('China_ann_prcp.pkl', 'wb') as f:
        pickle.dump({'bigPr_ann_tr': bigPr_ann_tr, 
                     'bigPr_ann_pr': bigPr_ann_pr}, f)
