...
def load_data():
    bigPr_ground_tr = np.zeros((15330, Nlat, Nlon), dtype='float32')
    bigPr_poisson_tr = np.zeros((15330, Nlat, Nlon), dtype='float32')
    bigPr_ground_pr = np.zeros((5110, Nlat, Nlon), dtype='float32')
    bigPr_poisson_pr = np.zeros((5110, Nlat, Nlon), dtype='float32')
    for j in range(Nlat):
        for i in range(Nlon):
            if m[j, i]:
                dataid = "%05i" % (j * Nlon + i)
                print("%s: In China！！j=%i, i=%i" % (dataid, j, i))
                fname = "./out/poisson_data." + dataid
                with sh.open(fname) as f:
                    # 1. read data from the tuple:(xCp, xGp, xGp_linear)
                    data_poisson_pr = f['data_predict'][2]
                    data_poisson_tr = f['data_train'][2]
                # 2. put into Big array
                bigPr_poisson_tr[:, j, i] = data_poisson_tr[:]
                bigPr_poisson_pr[:, j, i] = data_poisson_pr[:]

    return bigPr_poisson_tr, bigPr_poisson_pr
...
if __name__ == '__main__':
    ...
    # Combine
    bigPr_poisson_tr, bigPr_poisson_pr = load_data()

    # Dump and Load
    with open('China_poisson_prcp.pkl', 'wb') as f:
        pickle.dump({'bigPr_poisson_tr': bigPr_poisson_tr,                 
                     'bigPr_poisson_pr': bigPr_poisson_pr}, f)
